...
func (srv *SearchClient) FindUsers(req SearchRequest) (*SearchResponse, error) {
	...
	if req.Limit < 0 {
		return nil, fmt.Errorf("limit must be > 0") //это покрыто
	}
	if req.Limit > 25 {
		req.Limit = 25
	}
	if req.Offset < 0 {
		return nil, fmt.Errorf("offset must be > 0") //это покрыто
	}
	...
	if err != nil {
		if err, ok := err.(net.Error); ok && err.Timeout() {
			return nil, fmt.Errorf("timeout for %s", searcherParams.Encode()) //это покрыто
		}
		return nil, fmt.Errorf("unknown error %s", err) //это покрыто
	}
